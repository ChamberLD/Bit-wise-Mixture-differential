...
def genVars_Round(r):
    return ['L_'+str(r)+'r_'+str(i) for i in range(48)] + ['R_'+str(r)+'r_'+str(i) for i in range(48)]

#每一轮的左移1位的bit的混合差分模式
def genVars_SR1_Round(r):
    return ['SR1_'+str(r)+'r_'+str(i) for i in range(48)]

#每一轮的左移2位的bit的混合差分模式
def genVars_SR2_Round(r):
    return ['SR2_'+str(r)+'r_'+str(i) for i in range(48)]

#每一轮的左移8位的bit的混合差分模式
def genVars_SR8_Round(r):
    return ['SR8_'+str(r)+'r_'+str(i) for i in range(48)]

#每一轮的＆运算之后的bit的混合差分模式
def genVars_aftAnd_Round(r):
    return ['aftAnd_'+str(r)+'r_'+str(i) for i in range(48)]

#每一轮的第一次异或后的bit的混合差分模式
def genVars_aftXOR1_Round(r):
    return ['aftXOR1_'+str(r)+'r_'+str(i) for i in range(48)]

#每一轮的and操作后的混合差分概率变量
def genVars_ProaftAnd_Round(r):
    return ['ProaftAnd_'+str(r)+'r_'+str(i) for i in range(48)]

#生成每一轮需要的copy变量，copy变量在传递约束方面是很有用的
def genVars_Copy_Round(r, k):
    return ['Copy_'+str(k)+'_'+str(r)+'r_'+str(i) for i in range(48)]

#生成每一轮需要的概率变量ph的copy变量
def genVars_PCopy_Round():
    #用p_number来表示概率变量的下标
    global  p_number
    return ['PCopy_'+str(i) for i in range(p_number, p_number+48)]
...
def gen_XORCNF_Constraint(a, b, c):
    global number
    for i in range(0, 48):
        dict[c[i] + str("_c2")] = number; number = number + 1                        #在这里已经把下一轮的Lin的变量的位置数值给确定了，下一轮是不是就不用确定了
        dict[c[i] + str("_c1")] = number; number = number + 1
        dict[c[i] + str("_c0")] = number; number = number + 1
        solver.add_clause([-dict[a[i] + str("_c0")], -dict[b[i] + str("_c0")], -dict[c[i] + str("_c0")]])   # c0'+d0'+e0'
        solver.add_clause([dict[a[i] + str("_c0")], dict[b[i] + str("_c0")], -dict[c[i] + str("_c0")]])     # c0+d0+e0'
...
def shift_left(a, b, k):
    if k == 1:
        for i in range(0, 48):
            dict[b[(i - 1)%48] + str("_c2")] = dict[a[i] + str("_c2")]
            dict[b[(i - 1)%48] + str("_c1")] = dict[a[i] + str("_c1")]
            dict[b[(i - 1)%48] + str("_c0")] = dict[a[i] + str("_c0")]
    elif k == 2:
        for i in range(0, 48):
            dict[b[(i - 2) % 48] + str("_c2")] = dict[a[i] + str("_c2")]
            dict[b[(i - 2) % 48] + str("_c1")] = dict[a[i] + str("_c1")]
            dict[b[(i - 2) % 48] + str("_c0")] = dict[a[i] + str("_c0")]
    else:
        for i in range(0, 48):
            dict[b[(i - 8) % 48] + str("_c2")] = dict[a[i] + str("_c2")]
            dict[b[(i - 8) % 48] + str("_c1")] = dict[a[i] + str("_c1")]
            dict[b[(i - 8) % 48] + str("_c0")] = dict[a[i] + str("_c0")]
            ...
            for m in range(0, length):
                ...
                for j in range(0, len(line_split)):
                    ...
                    if line_split[j] == "αy_c2":
                        constraint.append("dict[a[(i+7)%48] + str(\"_c2\")]")
                    if line_split[j] == "αy_c1":
                        constraint.append("dict[a[(i+7)%48] + str(\"_c1\")]")
                    if line_split[j] == "αy_c0":
                        constraint.append("dict[a[(i+7)%48] + str(\"_c0\")]")
                    if line_split[j] == "αy_c2'":
                        constraint.append("-dict[a[(i+7)%48] + str(\"_c2\")]")
                    if line_split[j] == "αy_c1'":
                        constraint.append("-dict[a[(i+7)%48] + str(\"_c1\")]")
                    ...
                    if line_split[j] == "αz_c2":
                        constraint.append("dict[a[(i+41)%48] + str(\"_c2\")]")
                    if line_split[j] == "αz_c1":
                        constraint.append("dict[a[(i+41)%48] + str(\"_c1\")]")
                    if line_split[j] == "αz_c0":
                        constraint.append("dict[a[(i+41)%48] + str(\"_c0\")]")
                    if line_split[j] == "αz_c2'":
                        constraint.append("-dict[a[(i+41)%48] + str(\"_c2\")]")
                    if line_split[j] == "αz_c1'":
                        constraint.append("-dict[a[(i+41)%48] + str(\"_c1\")]")
                    ...
                    if line_split[j] == "βxz_c2":
                        constraint.append("dict[c[(i+41)%48] + str(\"_c2\")]")
                    if line_split[j] == "βxz_c1":
                        constraint.append("dict[c[(i+41)%48] + str(\"_c1\")]")
                    if line_split[j] == "βxz_c0":
                        constraint.append("dict[c[(i+41)%48] + str(\"_c0\")]")
                    if line_split[j] == "βxz_c2'":
                        constraint.append("-dict[c[(i+41)%48] + str(\"_c2\")]")
                    if line_split[j] == "βxz_c1'":
                        constraint.append("-dict[c[(i+41)%48] + str(\"_c1\")]")
                    ...
                    if line_split[j] == "Pdep":
                        constraint.append("dict[d[(i-1)%48] + str(\"_Pdep\")]")
                    if line_split[j] == "Pdep'":
                        constraint.append("-dict[d[(i-1)%48] + str(\"_Pdep\")]")
                constraint_real = []
...
p_number = 1
#定义求解器
solver = pycryptosat.Solver(threads=64)
#主循环,这里的循环次数是想要找的混合差分区分器的轮数
for i in range(1, 30):
    Lin = genVars_Round(i)[0:48]
    Rin = genVars_Round(i)[48:96]
    ...
    if i == 1:
        ...
        copy(genVars_Round(i - 1)[0:48], Rin)  # 这里实际上就是R(r)和Lin(r-1)实际上指向的是同一个变量，通过这样可以减少相应的变量数目
    ...
    gen_XORCNF_Constraint(aft_XOR, SR2, genVars_Round(i+1)[0:48])
...
if sat == False:
    ...
    for i in range(1, 30):
        for j in range(0, 48):
            ...
            print("###########################################################")
    for k in range(0, 48):
        print("L_30r_" + str(k) + "_c2" +":  %s"%solution[dict["L_30r_" + str(k) + "_c2"]] +"  "
              "L_30r_" + str(k) + "_c1" +":  %s"%solution[dict["L_30r_" + str(k) + "_c1"]] +"  "
              "L_30r_" + str(k) + "_c0" +":  %s"%solution[dict["L_30r_" + str(k) + "_c0"]])
    print(count)
