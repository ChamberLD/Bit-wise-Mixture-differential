"""
Function: Model SIMON algorithm and find Mixture differential distinguishers   /////Supplementary experiments for SIMON48-96
Date: 2022/07/14
Author: Zehan Wu
Contact:
"""
...
def genVars_Round(r):
    return ['L_'+str(r)+'r_'+str(i) for i in range(24)] + ['R_'+str(r)+'r_'+str(i) for i in range(24)]

#每一轮的左移1位的bit的混合差分模式
def genVars_SR1_Round(r):
    return ['SR1_'+str(r)+'r_'+str(i) for i in range(24)]

#每一轮的左移2位的bit的混合差分模式
def genVars_SR2_Round(r):
    return ['SR2_'+str(r)+'r_'+str(i) for i in range(24)]

#每一轮的左移8位的bit的混合差分模式
def genVars_SR8_Round(r):
    return ['SR8_'+str(r)+'r_'+str(i) for i in range(24)]

#每一轮的＆运算之后的bit的混合差分模式
def genVars_aftAnd_Round(r):
    return ['aftAnd_'+str(r)+'r_'+str(i) for i in range(24)]

#每一轮的第一次异或后的bit的混合差分模式
def genVars_aftXOR1_Round(r):
    return ['aftXOR1_'+str(r)+'r_'+str(i) for i in range(24)]

#每一轮的and操作后的混合差分概率变量
def genVars_ProaftAnd_Round(r):
    return ['ProaftAnd_'+str(r)+'r_'+str(i) for i in range(24)]

#生成每一轮需要的copy变量，copy变量在传递约束方面是很有用的
def genVars_Copy_Round(r, k):
    return ['Copy_'+str(k)+'_'+str(r)+'r_'+str(i) for i in range(24)]

#生成每一轮需要的概率变量ph的copy变量
def genVars_PCopy_Round():
    #用p_number来表示概率变量的下标
    global  p_number
    return ['PCopy_'+str(i) for i in range(p_number, p_number+24)]
...
def shift_left(a, b, k):
    if k == 1:
        for i in range(0, 24):
            dict[b[(i - 1) % 24] + str("_c2")] = dict[a[i] + str("_c2")]
            dict[b[(i - 1) % 24] + str("_c1")] = dict[a[i] + str("_c1")]
            dict[b[(i - 1) % 24] + str("_c0")] = dict[a[i] + str("_c0")]
    elif k == 2:
        for i in range(0, 24):
            dict[b[(i - 2) % 24] + str("_c2")] = dict[a[i] + str("_c2")]
            dict[b[(i - 2) % 24] + str("_c1")] = dict[a[i] + str("_c1")]
            dict[b[(i - 2) % 24] + str("_c0")] = dict[a[i] + str("_c0")]
    else:
        for i in range(0, 24):
            dict[b[(i - 8) % 24] + str("_c2")] = dict[a[i] + str("_c2")]
            dict[b[(i - 8) % 24] + str("_c1")] = dict[a[i] + str("_c1")]
            dict[b[(i - 8) % 24] + str("_c0")] = dict[a[i] + str("_c0")]
            ...
            for m in range(0, length):
                ...
                for j in range(0, len(line_split)):
                    ...
                    if line_split[j] == "αy_c2":
                        constraint.append("dict[a[(i+7)%24] + str(\"_c2\")]")
                    if line_split[j] == "αy_c1":
                        constraint.append("dict[a[(i+7)%24] + str(\"_c1\")]")
                    if line_split[j] == "αy_c0":
                        constraint.append("dict[a[(i+7)%24] + str(\"_c0\")]")
                    if line_split[j] == "αy_c2'":
                        constraint.append("-dict[a[(i+7)%24] + str(\"_c2\")]")
                    if line_split[j] == "αy_c1'":
                        constraint.append("-dict[a[(i+7)%24] + str(\"_c1\")]")
                    ...
                    if line_split[j] == "αz_c2":
                        constraint.append("dict[a[(i+17)%24] + str(\"_c2\")]")
                    if line_split[j] == "αz_c1":
                        constraint.append("dict[a[(i+17)%24] + str(\"_c1\")]")
                    if line_split[j] == "αz_c0":
                        constraint.append("dict[a[(i+17)%24] + str(\"_c0\")]")
                    if line_split[j] == "αz_c2'":
                        constraint.append("-dict[a[(i+17)%24] + str(\"_c2\")]")
                    if line_split[j] == "αz_c1'":
                        constraint.append("-dict[a[(i+17)%24] + str(\"_c1\")]")
                    ...
                    if line_split[j] == "βxz_c2":
                        constraint.append("dict[c[(i+17)%24] + str(\"_c2\")]")
                    if line_split[j] == "βxz_c1":
                        constraint.append("dict[c[(i+17)%24] + str(\"_c1\")]")
                    if line_split[j] == "βxz_c0":
                        constraint.append("dict[c[(i+17)%24] + str(\"_c0\")]")
                    if line_split[j] == "βxz_c2'":
                        constraint.append("-dict[c[(i+17)%24] + str(\"_c2\")]")
                    if line_split[j] == "βxz_c1'":
                        constraint.append("-dict[c[(i+17)%24] + str(\"_c1\")]")
                    ...
                    if line_split[j] == "Pdep":
                        constraint.append("dict[d[(i-1)%24] + str(\"_Pdep\")]")
                    if line_split[j] == "Pdep'":
                        constraint.append("-dict[d[(i-1)%24] + str(\"_Pdep\")]")
                constraint_real = []
...
for i in range(1, 19):
    Lin = genVars_Round(i)[0:24]
    Rin = genVars_Round(i)[24:48]
    ...
    if i == 1:
        ...
        copy(genVars_Round(i - 1)[0:24], Rin)  # 这里实际上就是R(r)和Lin(r-1)实际上指向的是同一个变量，通过这样可以减少相应的变量数目
    ...
    gen_XORCNF_Constraint(aft_XOR, SR2, genVars_Round(i+1)[0:24])       #
...
if sat == False:
    ...
    for i in range(1, 19):
        for j in range(0, 24):
            ...
            print("###########################################################")
    for k in range(0, 24):
        print("L_19r_" + str(k) + "_c2" +":  %s"%solution[dict["L_19r_" + str(k) + "_c2"]] +"  "
              "L_19r_" + str(k) + "_c1" +":  %s"%solution[dict["L_19r_" + str(k) + "_c1"]] +"  "
              "L_19r_" + str(k) + "_c0" +":  %s"%solution[dict["L_19r_" + str(k) + "_c0"]])
    print(count)
